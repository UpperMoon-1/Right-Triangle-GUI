...
public class RTSameView extends JPanel {
	...
	private class ButtonListener implements ActionListener {
		...
		public void actionPerformed(final ActionEvent evt) {
			// your code here
			if (direction == 0) {
				double average = (model.getBase() + model.getHeight()) / 2;
				// System.out.println(model.getBase());
				// System.out.println(model.getHeight());
				// System.out.println(average);
				model.setBase(average);
				model.setHeight(average);
			} else {
				model.setBase(model.getBase() + this.direction);
				model.setHeight(model.getHeight() + this.direction);
			}
		}
	}
	...
	private class ValuesListener implements PropertyChangeListener {
		...
		public void propertyChange(final PropertyChangeEvent evt) {
			// your code here
			if (model.getBase() == model.getHeight()) {// RTSameView.DECIMAL_FORMAT.format(RTSameView.this.mode.getHeight()));
				baseHeight.setText(String.format("%.0f", model.getHeight()));// set it to the base
			} else {
				baseHeight.setText("");
			}
			hypo.setText(String.format("%.0f", model.getHypotenuse()));
		}
	}
